...
int main(){
    ...
    // i also took in consideration the min cost and min char if both characters are equal 
        ...
        /*
        abc??def?gh
        4 9 5 9 6 f1 g0 3 7 2 5 9 6 1 3 2 3 2 9 1 1 0 1 8 8 4
    ...
    for(int i=0; i<(int)s.size()-1; i++) {
        ...
        if(s[i]=='?'){
            ...
            for(int j=1; j< 26; j++)
                ...
                if(mincost > min(mincost, abs( cost[s[i-1]-'a'] - cost[j] )) ||  (mincost == min(mincost, abs( cost[s[i-1]-'a'] - cost[j] )) && s[i] < char(j+97)))
                        s[i] = char(j+97) ;
